...
| **无法验证 = 无法交付** | 服务端依赖 `fibers`（当前 Node 无法加载原生模块）与真实 MySQL，客户端需要图形化编辑器构建，2016 年的 `tests/*.js` 只会打印不断言 | `npm run verify`：零依赖、可离线跑的五项门禁 |
...
**"编译但不执行"是关键设计**：`server` 里有 `require('fibers')`，执行就会炸。
用 `vm.Script` 只编译，既拿到语法错误的全部价值，又不需要任何原生依赖或数据库。
...
| 项目 | 原因 |
| --- | --- |
| 服务端运行时行为 | 账号服 `require('fibers')` 缺原生二进制；`db.js` 需要真实 MySQL |
