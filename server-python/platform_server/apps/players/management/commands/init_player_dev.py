...
**对局记录只读归档表** `t_games_archive`（见 `player_source.py` 对局记录那一段），
所以样例里刻意留了一个"只在在局表里"的房间，用来验证它**不会**被后台查出来：

======================  ===============================  ==============================
房间                    样例内容                          对局记录里的表现
======================  ===============================  ==============================
``526035``（未销毁）     在局 2 局（只在 `t_games`，未归档） **查不到**（正确行为）
``641280``（已销毁）     归档 3 局 + 战绩快照              3 局；身份来自 `t_users.history`
``593071``（已销毁）     归档 1 局、**不写**战绩快照         1 局；身份查不到 → `unknown`
======================  ===============================  ==============================

`641280` / `593071` 都**不在** `t_rooms` 里（房间已销毁），所以玩家身份只能靠
`t_users.history` 反查：`641280` 有四家快照，`593071` 没有（游戏服只在
`numOfGames > 1` 时写快照），后者在后台显示成 `座位N`。
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> str:
        ...
        with connection.cursor() as cursor:
            ...
            #    对局记录只读归档表，所以这些行**不该**被后台查出来——它们是样例里的负例。
