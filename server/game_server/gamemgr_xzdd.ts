 ...
 *
 * 与姐姐文件 `gamemgr_xlch.ts`（血战到底）约 86% 逐行相同，差异集中在番型判定与流程分支
        ...
        if(isTinged(sd)){
            ...
            if(game.conf.jiangdui){
                if(sd.pattern == "7pairs"){
                    if(sd.numofgen > 0){
                        ...
                        //`as string` 只是让编译器别把 sd.pattern 收窄成字面量 "7pairs"。
                        (sd.pattern as string) == "l7pairs";
                        isjiangdui = isJiangDui(sd);
                        if(isjiangdui){
                            (sd.pattern as string) == "j7paris";
                            fan += 2;    
                        }   
                        else{
                            fan += 1;
            ...
            for(var a = 0; a < sd.actions.length; ++a){
                ...
                if(ac.type == "fanggang"){
                    ...
                    //原实现直接传 ac.owner（没有判空）：owner 为 null 时 isTinged 会因读
                    //seatData.tingMap 抛 TypeError。这里用非空断言保留该行为——maozhuanyu 记录
                    //创建时 owner 一定有值，null 只在本分支末尾写入；断言只影响类型。
...
interface UserResult {
    ...
    dihu: boolean;
    huorder: number;
    /** xzdd 的结算项不带 huinfo（xlch 才带），故为可选。 */
    huinfo?: HuInfoItem[];
}
...
interface SyncSeat {
    ...
    que: number;
    hued: boolean;
    /** xzdd 的同步座位不带 huinfo（xlch 才带），故为可选。 */
    huinfo?: HuInfoItem[];
    iszimo: boolean;
    /** 只有自己的座位才带上手牌，原实现如此。 */
    holds?: number[];
...
interface SyncData {
    ...
    turn: number;
    chuPai: number;
    /** xzdd 独有：本局换三张的换牌方式（0 对家 / 1 下家 / 2 上家）。 */
