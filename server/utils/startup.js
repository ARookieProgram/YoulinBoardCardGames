...
function endpointUrl(ep){
	...
	var port = (addr && addr.port) ? addr.port : ep.port;
	return ep.scheme + '://' + displayHost(ep.host) + ':' + port;
}

function endpointPort(ep){
	var addr = ep.server.address();
	return (addr && addr.port) ? addr.port : ep.port;
}
...
function printBanner(options,dbStatus){
	...
	for(var j = 0; j < rows.length; ++j){
		if(rows[j] !== null && displayWidth(rows[j]) + 2 > width){
			width = displayWidth(rows[j]) + 2;
		}
	}
	if(width < MIN_WIDTH){
		width = MIN_WIDTH;
...
function printFailure(options,ep,err){
	var lines = [];
	lines.push('');
	lines.push('启动失败：' + options.title);
	lines.push('  端点     : ' + ep.label);
	lines.push('  地址     : ' + endpointUrl(ep));
	lines.push('  错误     : ' + ((err && err.code) ? err.code + ' ' : '') + ((err && err.message) ? err.message : err));
