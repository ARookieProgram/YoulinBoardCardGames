...
* **对局记录**（**只读归档表 `t_games_archive`**：房间打完 / 被解散后才归档，
  所以看到的每一局都是终局）：归档列表（关键字 / 玩法 / 日期区间 / 排序 / 分页）+ 概览 +
  一个房间的全部归档对局 + **单局出牌记录**（全局时间线 / 分每个玩家的出牌顺序 /
  开局手牌与牌墙）+ 某个玩家的最近战绩；
* 控制台。
...
> 的只读数据源（`t_users` / `t_rooms` / `t_games_archive`），
...
│   └─ games.ts            对局记录接口（归档列表 / 概览 / 房间对局 / 单局出牌记录 / 玩家战绩）
