/**
 * 快捷语条目：Chat.ts 里存的是 `content` 字段，共享的 QuickChatInfo 里对应字段叫 `text`。
 * 这里按 Chat.ts 实际写出的字段继承一份（只在本模块可见），
 * 这样 `cc.vv.chat = this` 依旧满足 cc-vv.d.ts 里的 ChatApi。
 */
interface QuickChatItem extends QuickChatInfo {
    content: string;
}
