...
var encodermap: { [key: number]: string } = {}
var decodermap: { [key: string]: number } = {}
for(var i = 0; i < 256; ++i){
    var code = null;
    var v = i + 1;
    if(v >= base){
...
function getCode(content: string,index: number){
    // c 先存字符码（数字）用于比较，两个分支随后都覆盖成字符串，返回值因此是 string
    var c: number | string = content.charCodeAt(index);
    if(c >= base){
...
cc.Class({
    ...
    writeVoice:function(filename: string,voiceData: unknown){
        ...
        // voiceData 是网络载荷（voice_msg 里的编码字符串），在这里的动态边界断言成 string
        if(voiceData && (voiceData as string).length > 0){
            var fileData = decode(voiceData as string);
    ...
    getVoiceLevel:function(maxLevel: number): number{
        ...
        if(cc.sys.os == cc.sys.OS_ANDROID){ 
            return jsb.reflection.callStaticMethod("com/babykylin/VoiceRecorder", "getVoiceLevel", "(I)I",maxLevel) as number;
        }
        else if(cc.sys.os == cc.sys.OS_IOS){
        }
        else{
